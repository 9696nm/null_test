...
# Alpine
docker build -f dockerfiles/Dockerfile.alpine -t nulltest-alpine .
docker run --rm nulltest-alpine > logs/alpine.log 2>&1

# TinyCC
docker build -f dockerfiles/Dockerfile.tinycc -t nulltest-tinycc .
docker run --rm nulltest-tinycc > logs/tinycc.log 2>&1

# Clang 16
docker build -f dockerfiles/Dockerfile.clang16 -t nulltest-clang16 .
docker run --rm nulltest-clang16 > logs/clang16.log 2>&1

# MinGW (Windows)
docker build -f dockerfiles/Dockerfile.mingw -t nulltest-mingw .
